...
OUTPUT_DIR  = '/kaggle/working' if os.path.exists('/kaggle') else 'result/results_DT_Spirit'
DS_KEY      = 'spirit'
...
print(f"{'='*60}")
print(f"  Decision Tree – Spirit (standalone)")
print(f"{'='*60}")
...
if 'data_ready' not in ckpt:
    ...
    print('[1-2/4] Rebuilding data (sparse matrices not persisted) …')
    X_train, y_train, X_val, y_val, X_test, y_test, tfidf = build_data()
...
if 'dt_done' not in ckpt:
    ...
    print('[3/4] DT already trained – loading …')
...
ax.set_title(f'DT Spirit – Confusion Matrix\nF1={test_f1:.4f}')
for i in range(2):
    for j in range(2):
...
ax.set_title(f'Top-{top_k} DT Feature Importances – Spirit')
plt.tight_layout()
...
try:
    ...
    ax.set_title(f'Top-{top_k} SHAP – DT Spirit  [Lundberg2017_SHAP]')
    plt.tight_layout()
...
# =============================================================================
print('\n' + '='*60)
print('CLASSIFICATION REPORT – DT Spirit')
print('='*60)
