# =============================================================================
# STANDALONE NOTEBOOK 09 — CNN+BiLSTM Hybrid on Spirit (Fully Independent)
#
...
# Based on [Zhang2019_LogRobust] — Attention-based BiLSTM for robust anomaly
#   detection; bidirectional context improves recall on Spirit.
# Based on [Bekkouche2025_Spirit] — Spirit sliding-window strategy;
#   WINDOW_SIZE=20, STEP_SIZE=10 balances context vs. label resolution.
#
# Architecture: Embedding → Multi-Scale CNN (k=[2,3,5]) → BiLSTM → Attention
#               → Dropout → FC (Binary)
#
# ✅ ZERO dependencies — reads Spirit_Drain.csv directly from Kaggle input.
# ✅ Builds vocab + sliding-window sessions inline (no external .npz needed).
# ✅ One dataset only → RAM stays safe on Kaggle.
# ✅ Checkpoint system: 'sessions_ready', 'done'
#
# Kaggle setup:
...
BASE_OUT  = '/kaggle/working'               if KAGGLE else 'result/results_CNNBiLSTM_Spirit'
...
# Safety cap — set to e.g. 3_000_000 if OOM occurs on Kaggle
...
if 'sessions_ready' not in ckpt:
    print(f"\n{'='*65}")
    print(f"  📂 CELL 2 — Spirit Sliding-Window Session Building")
    print(f"{'='*65}")
    t0 = time.time()
    ...
    # ── Chunked load: collect only template + label columns ──────────────────
    all_templates = []
    all_labels    = []
    rows_loaded   = 0
    ...
    # ── Build vocabulary (fit on TRAIN+VAL corpus to avoid data leakage) ──────────
    ...
    # ── Convert templates to integer token sequences ─────────────────────────
    ...
    # ── Build sliding-window sequences on entire log stream first ─────────────
    ...
    print(f"  ✅ Session building done ({time.time()-t0:.0f}s)")
...
class MultiScaleCNNBiLSTM(nn.Module):
    ...
    CNN branch captures local n-gram patterns at kernels k∈{2,3,5}.
    ...
    def __init__(self, vocab_size, embed_dim=64, cnn_filters=64,
        ...
        # Multi-scale 1D CNN — different kernels = different n-gram scales
        ...
        # Soft attention — [Zhang2019_LogRobust]
...
# =============================================================================
# CELL 4 — Training Function (mixed precision, pos_weight, cosine scheduler)
# =============================================================================
...
def train_cnn_bilstm(X_tr, y_tr, X_v, y_v, X_te, y_te,
    ...
    # ── Test inference — threshold from val, not from test ───────────────────
...
# =============================================================================
if 'done' in ckpt:
    print("\n[CELL 5] ⏭️  Already done (checkpoint). Skipping training.")
else:
    print(f"\n{'='*65}")
    print(f"  🔬 SPIRIT CNN+BiLSTM OPTIMIZATION [Lu2018 + Zhang2019]")
    print(f"{'='*65}")
    t0_total = time.time()
    ...
    print(f"\n  🚀 Full training (40 epochs, patience=10) ...")
    ...
    print(f"\n  📊 TEST RESULTS — Spirit CNN+BiLSTM:")
    ...
    a1.set_title('Loss — Spirit CNN+BiLSTM'); a1.set_xlabel('Epoch')
    ...
    a2.set_title('Validation F1 — Spirit CNN+BiLSTM')
    ...
    ax.set_title('Confusion Matrix — Spirit CNN+BiLSTM (Opt)')
    ...
    plt.title('ROC — Spirit CNN+BiLSTM'); plt.legend(); plt.grid(alpha=0.3)
    plt.tight_layout()
    ...
    print(f"\n  ✅ Spirit CNN+BiLSTM done ({time.time()-t0_total:.0f}s)")
...
# =============================================================================
print(f"\n{'='*65}")
print(f"  ✅ SPIRIT CNN+BiLSTM STANDALONE — VERIFICATION")
print(f"{'='*65}")
...
print(f"\n  {'🎉 All outputs verified!' if all_ok else '⚠️  Some outputs missing.'}")
