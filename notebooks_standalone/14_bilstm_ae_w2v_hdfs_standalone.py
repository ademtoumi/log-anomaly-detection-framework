# =============================================================================
# STANDALONE NOTEBOOK 14 — BiLSTM Autoencoder + Word2Vec on HDFS
#
...
#   → BiLSTM-AE + Word2Vec (Complete HDFS): F1=0.993, Precision=0.987, Recall=1.000
#
# KEY DISTINCTION from Notebook 12 (lstm_ae_hdfs):
#   Notebook 12  — uses nn.Embedding (random init, learned end-to-end)
#   Notebook 14  — uses Word2Vec trained from scratch on ALL HDFS sessions
...
#
# Architecture: [Bekkouche2025_BiLSTM] §III-B
#   W2V Embedding (fine-tuned, dim=100)
#   → BiLSTM Encoder (hidden=128, bidirectional=True)
#       last hidden state h_n → concat fwd+bwd → Linear → tanh → latent (hidden)
#   → LSTM Decoder: latent as initial hidden state h0, zero input sequence
#   → Linear projection → embedding_dim (reconstructed sequence)
#   Loss: MSELoss(decoded, original_embeddings.detach())
#
# Training protocol: [Bekkouche2025_BiLSTM] §III-C
#   • NORMAL sessions only (unsupervised)
#   • Adam optimiser, gradient clipping max_norm=1.0
#   • AMP (autocast + GradScaler) — T4/P100 GPU memory efficiency
#   • CosineAnnealingLR (primary) + ReduceLROnPlateau (secondary, factor=0.5, patience=5)
#   • Early stopping on val MSE (patience=15) — val = 10% hold-out of train (normal only)
#
# Threshold protocol: [Bekkouche2025_BiLSTM] §II-C-a (EXACT PAPER PROTOCOL)
#   • Threshold searched directly on the TEST set to maximise F1-Score
#   • Quote: "the test partition of the dataset is used to determine the
#     threshold value that maximises the F1-Score" — semi-supervised threshold
#   • Range: [10th-percentile of normal test errors, 99.5th-percentile of all test errors]
#   • 5000 candidates for fine-grained search
#
# Split protocol: [Bekkouche2025_BiLSTM] Table I — EXACT PAPER PROTOCOL
#   • Uniform split (stratified random): 90% train / 10% test
#   • sklearn train_test_split(stratify=y, test_size=0.10, random_state=42)
#   • Paper reports: 517,554 train / 57,507 test (Complete HDFS)
#   • NO separate validation set for threshold — threshold tuned on TEST directly
#
# Session protocol: [Du2017_DeepLog]
#   • Sessions grouped by BlockId extracted via regex blk_-?\d+
#   • Label: any anomalous line in session → session label = 1
#   • MAX_SEQ_LEN computed from actual data (median + 2*MAD, capped at 100)
#
# Memory strategy:
#   • Chunked CSV reading (chunksize=50_000) — safe for 16 GB Kaggle RAM
#   • Two-pass approach: Pass 1 collects sessions; Pass 2 trains W2V corpus
#   • gc.collect() + cuda.empty_cache() after every major step
#   • int32 sequences + float32 embeddings throughout
#
# Checkpoint system:
#   • ckpt_14_bilstm_ae_hdfs.json — full resume after Kaggle timeout
#   • Guards: 'sessions_ready', 'w2v_ready', 'ae_done'
#
# Reproducibility:
#   • Fixed seed=42: numpy, random, torch, Word2Vec (workers=1)
#   • Environment logged in summary cell
#
# Kaggle setup:
...
#   models/
#     w2v_hdfs_full.model          — trained Word2Vec model
#     bilstm_ae_w2v_hdfs_opt.pt   — best AE weights
...
import pandas as pd
import matplotlib
matplotlib.use('Agg')   # non-interactive backend — safe for Kaggle commit mode
...
BASE_OUT  = '/kaggle/working'               if KAGGLE else 'result/results_BiLSTMAE_W2V_HDFS_v1'
...
print(f"   BASE_IN  → {BASE_IN}")
print(f"   BASE_OUT → {BASE_OUT}")
print(f"   REPORT   → {REPORT}")

# ── Checkpoint system — full resume after Kaggle timeout ─────────────────────
...
ckpt = load_ckpt()
if not ckpt:
    print("   CHECKPOINT CLEARED — FRESH RUN")
else:
...
# =============================================================================
# CELL 2 — HDFS Session Building (inline, chunked)
#
# Protocol: [Du2017_DeepLog] §4.1
...
#
# Label strategy: [Bekkouche2025_BiLSTM] §III-A
#   A session is anomalous if ANY of its log lines is anomalous.
#   (label ≠ 'Normal' → anomalous line)
#
# Temporal split: [Zhang2019_LogRobust] §5.2
...
#
# Memory: chunked reading (50_000 rows) — safe for 16 GB Kaggle RAM.
#   Two dictionaries:
#     block_events[bid] → ordered list of template strings
#     block_labels[bid] → int: max anomaly flag seen (0 or 1)
#   Temporal order preserved via block_order list (insertion order).
# =============================================================================
CHUNK_SIZE  = 50_000     # rows per chunk — conservative for 16 GB RAM

# Compute MAX_SEQ_LEN from data statistics (not hardcoded)
# We use median + 2 * MAD, capped at 100, to be robust to outliers.
# [Du2017_DeepLog]: most HDFS sessions are 10–50 events.
...
if 'sessions_ready' in ckpt:
    ...
    # ── Pass 1: chunked session aggregation ──────────────────────────────────
    # block_events[bid] → list[str]  (template strings in arrival order)
    # block_labels[bid] → int        (1 if any anomalous line, else 0)
    # block_order       → list[str]  (insertion order ≈ temporal order)
    block_events: dict = {}
    block_labels: dict = {}
    block_order:  list = []
    ...
    for chunk in pd.read_csv(
        ...
        if 'BlockId' in chunk.columns:
            ...
            # Extract from raw log text — [Du2017_DeepLog] protocol
        ...
        if 'Label' in chunk.columns:
            ...
            chunk['_anom'] = 0   # no label column → treat all as normal
    ...
    print(f"\n  ✅ Pass 1 complete: {rows_total:,} rows → {len(block_order):,} sessions")
    ...
    # ── Compute MAX_SEQ_LEN from data (robust, not hardcoded) ────────────────
    # [Du2017_DeepLog]: HDFS sessions are mostly 10–50 events.
    ...
    # ── Build vocabulary from ALL templates ──────────────────────────────────
    # Vocab maps template → integer index.
    ...
    if abs((VOCAB_SIZE - 2) - 48) > 5:
        print(f"  ⚠️  Template count differs from paper's 48. Possible causes:")
        print(f"       • Drain parsing produced different granularity templates")
        print(f"       • Our CSV may merge/split some log event types")
        print(f"       • Paper may count EventId not template strings — acceptable difference")
    del all_templates, sorted_templates
    gc.collect()

    # ── Encode sequences as padded int32 arrays ──────────────────────────────
    ...
    # Free the heavy session dicts — we only need sequences + labels_arr now
    del block_events, block_labels
    gc.collect()
    print(f"  Sequences shape: {sequences.shape} | dtype={sequences.dtype}")

    # ── PRIMARY split: Stratified random 90/10 [Bekkouche2025_BiLSTM] Table I ──
    # Paper: "Uniform Split" — 517,554 train / 57,507 test on Complete HDFS
    ...
    # ── SECONDARY split: Temporal 60/20/20 (our rigorous protocol) ─────────────
    # Kept for thesis scientific rigour — no temporal leakage
    ...
    # Save sessions for checkpoint resume (primary split only — temp computed on-the-fly)
    ...
    save_ckpt(ckpt)
    print(f"\n  ✅ Sessions saved in {elapsed:.0f}s")
    print(f"     → {HDFSPath_NPZ_train}")
    print(f"     → {HDFSPath_NPZ_test}")
...
# =============================================================================
# CELL 2b — Anomaly Distribution Analysis (runs only on fresh build)
#
...
# If deciles 1–6 have much higher anomaly rate than decile 10 (test slice),
# the temporal 60/20/20 split produces a test set with fewer anomalies
# → harder evaluation, but incomparable to paper numbers from random splits.
#
...
if 'block_order' in dir() and 'block_labels' in dir() and block_order:
    # Fresh run — block_order and block_labels are still in memory
    ...
    for _d in range(10):
        ...
        print(f"    Decile {_d+1:2d} ({_s:>8,}–{_e:>8,}): "
    ...
    print(f"      Train (0–{_i1:,})        : {_anom_train:.2f}% anomaly")
    print(f"      Val   ({_i1:,}–{_i2:,})  : {_anom_val:.2f}% anomaly")
    print(f"      Test  ({_i2:,}–{_n:,})   : {_anom_test:.2f}% anomaly")
    if _anom_test < _anom_train * 0.6:
        print(f"\n    ⚠️  Anomalies ARE front-loaded — test set is anomaly-poor.")
        print(f"       → Recommend random stratified split for paper comparison.")
    else:
        print(f"\n    ✅ Anomaly distribution is roughly uniform across splits.")
    del _labels_full
else:
    print("    ℹ️  block_order not in memory (checkpoint resume) — "
          "re-run from scratch to see decile breakdown.")


# =============================================================================
# CELL 3 — Word2Vec Training (from scratch on the FULL HDFS corpus)
#
...
#   of ALL sessions — before any AE training starts.
#   This pre-trained semantic space is the core contribution
#   of [Bekkouche2025_BiLSTM].
#
# W2V Hyper-parameters: [Bekkouche2025_BiLSTM] §III-A
#   vector_size=100  — embedding dimension (paper default)
#   window=5         — context window
#   min_count=1      — keep ALL templates (no vocabulary pruning)
#   sg=1             — skip-gram (better for rare events [Meng2019])
#   workers=1        — deterministic training (SEED=42)
#   epochs=10        — sufficient for small log vocabulary
#
# Memory note: the W2V CORPUS is built from the int-encoded sequences
#   (list of lists of int strings) — RAM cost ≈ size of X_train_idx
#   which is already loaded above.
# =============================================================================
W2V_PATH = f'{MODEL_DIR}/w2v_hdfs_full.model'
EMBED_DIM = 100   # [Bekkouche2025_BiLSTM] §III-A

if 'w2v_ready' in ckpt:
    print("\n[CELL 3] ⏭️  Word2Vec already trained — loading model ...")
    ...
    elapsed = time.time() - t0
    print(f"  ✅ Word2Vec trained in {elapsed:.0f}s")
...
# =============================================================================
# CELL 4 — Build Embedding Matrix from Word2Vec (fine-tuned during AE training)
#
...
#
# Alignment: vocab index i → W2V vector for token str(i)
#   If str(i) not in W2V vocabulary → small random vector (rare edge case)
# =============================================================================
print(f"\n[CELL 4] Building embedding matrix ({VOCAB_SIZE} × {EMBED_DIM}) — will be fine-tuned ...")
...
# =============================================================================
# CELL 5 — BiLSTM Autoencoder Architecture
#
# [Bekkouche2025_BiLSTM] §III-B — architecture description:
#   Input: padded session of log-key indices, shape (B, T)
#   1. Embedding lookup (frozen W2V weights) → (B, T, 100)
#   2. BiLSTM Encoder: last hidden state (forward+backward concatenated)
#      → Linear(hidden*2, hidden) + tanh → latent (B, hidden)
#   3. Decoder: repeat latent T times → (B, T, hidden)
#      → LSTM Decoder (unidirectional — no future info at decode time)
#      → Linear(hidden, embed_dim=100) → reconstructed embeddings (B, T, 100)
#   Loss: MSE between original W2V embeddings and reconstructed embeddings
#
# Design choices:
#   • Bidirectional encoder: captures full context of the session sequence
#     for compression [Guo2021_LogBERT]
#   • Unidirectional decoder: causal (models reconstruction step-by-step)
#   • MSE on embedding space (not on logits): smooth, differentiable,
#     semantic proximity is meaningful in W2V space
#   • padding_idx=0 in embedding: PAD tokens map to zero vector, do not
...
class BiLSTMAutoencoderW2V(nn.Module):
    ...
    Encoder: Embedding (fine-tuned W2V) → BiLSTM → last hidden state → latent
    Decoder: initial hidden state from latent + positional encoding → LSTM → Linear → reconstructed embeddings
    ...
    def __init__(
        self,
        embedding_matrix: torch.Tensor,   # (vocab_size, embed_dim) — from W2V
        hidden_size: int  = 128,
        num_layers:  int  = 2,
        dropout:     float = 0.2,
    ):
        ...
        # Fine-tuned W2V embedding layer — weights ARE updated by backprop
        # Fine-tuned W2V embeddings for richer separation
        self.embedding = nn.Embedding.from_pretrained(
            embedding_matrix,
            freeze=False,         # IMPROVEMENT 1: unfreeze for fine-tuning
            padding_idx=0,        # PAD → zero vector, no gradient
        )
        self.embed_dim   = embed_dim
        self.hidden_size = hidden_size

        # BiLSTM Encoder [Bekkouche2025_BiLSTM] §III-B
        ...
        # Concatenate → Linear → tanh → latent of size hidden_size
        self.bottleneck = nn.Sequential(
            nn.Linear(hidden_size * 2, hidden_size),
            nn.Tanh(),
        )

        # LSTM Decoder (unidirectional — [Bekkouche2025_BiLSTM])
        ...
        # Replaces zero input — gives each timestep a unique signal so the
    ...
    def forward(self, x: torch.Tensor):
        ...
        Returns:
            original_emb: (B, T, embed_dim)  — W2V embeddings (detached targets)
            reconstructed: (B, T, embed_dim) — decoder output
        """
        B, T = x.size(0), x.size(1)

        # 1. Look up frozen W2V embeddings → (B, T, embed_dim)
        original_emb = self.embedding(x)

        # 2. BiLSTM Encoder → last hidden states
...
def train_ae_model(
    ...
    Schedulers:
      - CosineAnnealingLR (primary)   — smooth global decay
      - ReduceLROnPlateau (secondary) — aggressive LR cut on plateau
    ...
    # NOTE: verbose parameter removed — deprecated/dropped in PyTorch ≥ 2.2
    ...
    # Training DataLoader — normal sessions only
    train_ds = TensorDataset(torch.from_numpy(X_train_normal).long())
    train_dl = DataLoader(train_ds, batch_size=bs, shuffle=True,  num_workers=0,
                          pin_memory=(DEVICE.type == 'cuda'))

    # Validation DataLoader — normal val sessions only (for MSE monitoring)
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        # ── Validation pass (normal sessions only) ───────────────────────────
        model.eval()
        val_loss = 0.0
        with torch.no_grad():
            for (xb,) in val_dl:
...
# =============================================================================
# CELL 7 — Optuna Hyperparameter Search + Full Training
#
# Search space grounded in [Bekkouche2025_BiLSTM] + project conventions:
#   hidden_size : [128, 256, 512] — encoder/decoder hidden dim (512 added)
#   num_layers  : 1–2             — BiLSTM depth (3 layers overfits HDFS)
#   dropout     : 0.1–0.4         — regularisation
#   lr          : log[1e-4, 5e-3] — Adam learning rate
#   batch_size  : [128, 256, 512] — GPU-friendly
#
...
# Full training: 150 epochs max, patience=15 — sufficient for convergence.
#
...
if 'ae_done' in ckpt:
    print("\n[CELL 7] ⏭️  BiLSTM-AE already done (checkpoint 'ae_done') — skipping training.")

else:
    print(f"\n{'='*65}")
    print("  🧠 HDFS BiLSTM-AE + Word2Vec — OPTUNA + FULL TRAINING")
    print(f"{'='*65}")
    t0_total = time.time()

    # ── Load sessions (already built in CELL 2) ──────────────────────────────
    ...
    # Normal training sessions — unsupervised protocol [Bekkouche2025_BiLSTM]
    ...
    # ── Paper protocol verification [Bekkouche2025_BiLSTM] Table I ───────────
    ...
    print(f"  Train sequences : {_n_train:>8,}   (paper: 517,554)"
          f"  {'✅' if abs(_n_train - 517554) < 10000 else '⚠️ '}")
    print(f"  Test sequences  : {_n_test:>8,}   (paper:  57,507)"
          f"  {'✅' if abs(_n_test - 57507) < 2000 else '⚠️ '}")
    print(f"  Train anomalies : {_anom_train:>8,}   (paper:  15,154)"
          f"  {'✅' if abs(_anom_train - 15154) < 2000 else '⚠️ '}")
    print(f"  Test anomalies  : {_anom_test:>8,}   (paper:   1,684)"
          f"  {'✅' if abs(_anom_test - 1684) < 300 else '⚠️ '}")
    print(f"  Unique events   : {_unique_evts:>8,}   (paper:      48)"
          f"  {'✅' if abs(_unique_evts - 48) <= 10 else '⚠️  (Drain granularity diff)'}")
    print(f"  Threshold target: TEST SET  (matches paper §II-C-a)  ✅")
    print(f"{'='*51}")
    ...
    if _any_mismatch:
        print("\n  ⚠️  WARNING: One or more counts deviate significantly from Table I.")
        ...
        print("       Proceeding anyway — split ratio 90/10 and anomaly rate match the paper.\n")
    else:
        print("\n  ✅ All counts match Table I. Proceeding with paper-exact protocol.\n")

    # ── Small validation set for early stopping / Optuna (10% of normal train) ─
    # The paper has no separate val set — we hold out 10% of normal-only train
    ...
    X_train_opt = X_train_normal[_idx_nt]   # 90% of normal — actual training
    X_val_opt   = X_train_normal[_idx_nv]   # 10% of normal — early stopping only
    ...
    def objective(trial: optuna.Trial) -> float:
        cfg = {
            # 512 added — larger hidden dim helps on high-dim W2V space
            'hidden_size': trial.suggest_categorical('hidden_size', [128, 256, 512]),
            # 1–2 only — 3-layer BiLSTM overfits on HDFS normal sessions
        ...
        # ── Sample 50k sessions for fast Optuna trial ────────────────────────
        # Full X_train_opt is ~450k sessions — each trial would take ~8 min.
    ...
    print(f"\n  🔍 Optuna (30 trials, timeout=3600s, 50k sessions/trial) ...")
    study.optimize(objective, n_trials=30, timeout=3600,
                   show_progress_bar=False)
    # ── Retrieve best Optuna params (with safety fallback) ───────────────────
    ...
    if completed:
        ...
        # All trials failed/pruned — fall back to [Bekkouche2025_BiLSTM] paper defaults
        best_params = {
            'hidden_size': 128,
            'num_layers':  2,
            'dropout':     0.2,
        ...
        print(f"  ⚠️  All Optuna trials failed — using paper defaults: {best_params}")
        print(f"       Check the error messages above to diagnose the failure.")

    # ── Full training with best hyper-parameters ──────────────────────────────
    print(f"\n  🚀 Full training (max_epochs=150, patience=15) ...")
    t0_train = time.time()
    (model, best_state,
    ...
    print(f"     Normal train errors: μ={errors_train_normal.mean():.5f} σ={errors_train_normal.std():.5f}")
    print(f"     Test errors:         μ={errors_test.mean():.5f} σ={errors_test.std():.5f}")

    # ── [EVAL 1] Paper protocol — threshold tuned on TEST set ─────────────────
    # [Bekkouche2025_BiLSTM] §II-C-a:
    ...
    print(f"\n  [EVAL 1] Paper protocol — threshold search on TEST set (semi-supervised):")
    ...
    # Evaluate on test set — PAPER PROTOCOL
    ...
    metrics_paper = {
        ...
        'Threshold_Source':        'Test set (semi-supervised, per §II-C-a)',
    ...
    # ── [EVAL 2] Our rigorous protocol — temporal 60/20/20, blind test ─────────
    # Recompute errors on temporal splits (no reuse of primary model needed —
    ...
    # We need the full sequences array — load from train+test NPZ and reorder.
    print("\n  📐 Computing reconstruction errors (temporal 60/20/20 split) ...")
    # Reconstruct full ordered array from checkpoint NPZs
    if 'X_temp_train' not in dir():
        # Must recompute — load all data and apply temporal slices
        _all_X = np.concatenate([X_train_idx, X_test_idx], axis=0)   # note: shuffled by stratified split
        print("    ℹ️  Temporal arrays not in memory (checkpoint resume) —")
    ...
    print(f"\n  {'='*55}")
    print(f"  📊 EVAL 1 — Paper Protocol (90/10 strat., threshold on test):")
    print(f"  {'='*55}")
    ...
    if X_temp_test is not None:
        print(f"\n  {'='*55}")
        print(f"  📊 EVAL 2 — Our Protocol (temporal 60/20/20, blind test val):")
        print(f"  {'='*55}")
    ...
    # Gap comparison
    print(f"\n  📚 Paper target: F1=0.993, Precision=0.987, Recall=1.000")
    print(f"     EVAL 1 (paper protocol): F1={f1_paper:.4f} (Δ={f1_paper-0.993:+.4f})")
    if X_temp_test is not None:
        print(f"     EVAL 2 (our protocol):  F1={f1_temp:.4f} (Δ={f1_temp-0.993:+.4f})")
    ...
    # Save results CSV — paper protocol row first, our protocol second if available
    ...
    ax.set_title('BiLSTM-AE+W2V — Training Loss Curve\n[Bekkouche2025_BiLSTM]',
                 fontweight='bold')
    ax.set_xlabel('Epoch')
    ...
    # ── Plot 2: F1 vs Threshold Search Curves ──────────────────────────────────
    ax = axes[1]
    ...
    plt.close()
    print(f"  ✅ Loss curve → {PLOT_LOSS}")

    # ── Plot 3: Reconstruction Error Histogram (paper protocol test set) ───────
    ...
    ax.set_title('Reconstruction Error Distribution — HDFS Test Set (Paper Protocol)\n'
    ...
    plt.close()
    print(f"  ✅ Error histogram → {PLOT_HIST}")

    # ── Plot 4: Confusion Matrices (Paper protocol vs Temporal) ──────────────
    ...
    ax.set_title('ROC Curve — HDFS BiLSTM-AE+W2V\n[Bekkouche2025_BiLSTM]',
                 fontweight='bold')
    ...
    ax.set_title('Precision-Recall Curve — HDFS BiLSTM-AE+W2V\n[Bekkouche2025_BiLSTM]',
                 fontweight='bold')
    ...
    _rows += [
        ['BiLSTM-AE nn.Emb (NB12)', 'Unsupervised', '—', '—', '—'],
        ['DeepLog (NB13)',           'Unsupervised', '—', '—', '—'],
        ['Attn-BiLSTM (NB06)',      'Supervised',   '—', '—', '—'],
        ['CNN+BiLSTM (NB07)',        'Supervised',   '—', '—', '—'],
    ]
    ...
    print(f"\n  ✅ HDFS BiLSTM-AE+W2V COMPLETE — total wall time: "
          f"{total_time:.0f}s ({total_time/60:.1f} min)")

# =============================================================================
# CELL 9 — Verification Block
# =============================================================================
print(f"\n{'='*65}")
print("  ✅  NOTEBOOK 14 — BiLSTM-AE + W2V HDFS STANDALONE — COMPLETE")
print(f"{'='*65}")
...
for directory, fname, desc in expected_files:
    ...
    icon   = '✅' if exists else '❌'
    print(f"    {icon} {desc:<35s} → {fname}  {size_s}")
    if not exists:
        all_ok = False
...
# =============================================================================
# CELL 10 — Academic Summary
# =============================================================================
print(f"\n{'='*65}")
print("  📚  ACADEMIC SUMMARY — NOTEBOOK 14")
print(f"{'='*65}")
print(f"""
  ...
  Architecture:
    • Word2Vec: sg=1, dim={EMBED_DIM}, window=5, min_count=1
      trained from scratch on ALL HDFS sessions ({ckpt.get('vocab_size', '?')} unique templates)
    • Encoder: BiLSTM (bidirectional) — last hidden state → bottleneck
    • Decoder: LSTM (unidirectional) — latent as initial state + zero input → reconstruct
    • Loss: MSE in W2V embedding space (not on token IDs)
    • Training: NORMAL sessions only (unsupervised)
    • Threshold: F1-sensitive search on full val set (n_points=5000, range=[10th,99.5th pct])

  Split protocol [Bekkouche2025_BiLSTM] Table I:
    • PRIMARY  : Stratified random 90/10 (sklearn, random_state=42)
    • SECONDARY: Temporal 60/20/20 (no leakage) — our rigorous protocol
    • BlockId extraction: regex blk_-?\\d+
    • Label: any anomalous line → session label = 1
    • MAX_SEQ_LEN={ckpt.get('max_seq_len', '?')} (computed from data: median+2×MAD, cap=100)

  Threshold [Bekkouche2025_BiLSTM] §II-C-a:
    • Searched on TEST set directly (semi-supervised, F1-maximising)

  Results (Paper Protocol — 90/10 stratified, threshold on test):
    F1 (anomaly class)  : {ckpt.get('test_f1', '?')}
    ROC AUC             : {ckpt.get('test_auc', '?')}
    MCC                 : {ckpt.get('test_mcc', '?')}
    Threshold used      : {ckpt.get('threshold', '?')}

  Results (Our Protocol — 60/20/20 temporal, threshold on val):
  ...
  Gap analysis:
    Δ F1 (paper protocol) = {ckpt.get('test_f1', 0) - 0.993:+.4f}
...
if isinstance(test_f1_val, (int, float)):
    if test_f1_val >= 0.993:
        print("    ✅ REPRODUCTION SUCCESSFUL — Matches or exceeds paper F1=0.993.")
        ...
        print("    🟡 STRONG RESULT — Within 2.3pp of paper target (F1=0.993).")
        ...
        print("    🟠 ACCEPTABLE — F1 below paper by >2pp. Possible causes:")
        ...
        print("    🔴 BELOW PAPER — Significant gap. Likely causes:")
        print("       (1) Word2Vec not yet trained on full corpus (check w2v_ready ckpt),")
        print("       (2) MAX_SEQ_LEN too short for HDFS sessions,")
        print("       (3) Insufficient training epochs — increase to 100.")
        print("       Re-run with NROWS_LIMIT=None and GPU T4 for full-quality result.")
else:
    print("    ⚠️  Test F1 not yet recorded in checkpoint (training may not be complete).")
    print("       Check ckpt_14_bilstm_ae_hdfs.json for partial results.")

print(f"\n{'='*65}")
print("  References:")
print("    [Bekkouche2025_BiLSTM] Bekkouche M. et al. — BiLSTM-Autoencoder, IEEE 2025")
print("    [Du2017_DeepLog]       Du M. et al.         — DeepLog, CCS 2017")
print("    [Zhang2019_LogRobust]  Zhang X. et al.      — LogRobust, ESEC/FSE 2019")
print("    [Meng2019_LogAnomaly]  Meng W. et al.       — LogAnomaly, IJCAI 2019")
print("    [Guo2021_LogBERT]      Guo H. et al.        — LogBERT, IJCNN 2021")
print(f"{'='*65}")
