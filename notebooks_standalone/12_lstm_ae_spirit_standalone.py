# =============================================================================
# STANDALONE NOTEBOOK — BiLSTM Autoencoder on Spirit (Fully Independent)
#
# Based on [Bekkouche2025_BiLSTM] — Reconstruction error on normal samples
#   detects anomalies via MSE scoring. F1-sensitive threshold tuning is key.
#   BiLSTM-AE architecture (BiLSTM encoder, LSTM decoder) captures bidirectional context.
# Based on [Bekkouche2024] — Encoder-decoder architecture, MSE scoring.
#
# ✅ ZERO dependencies — reads raw Spirit_Drain.csv directly.
# ✅ Builds sliding-window sessions inline then frees the raw data.
# ✅ One dataset only → RAM stays safe on Kaggle.
#
# Architecture: Embedding → BiLSTM Encoder → LSTM Decoder → Projection
# Training: Normal sequences ONLY (unsupervised), MSE reconstruction loss
# Threshold: F1-sensitive grid search on validation set
#
# Kaggle setup:
...
BASE_OUT = '/kaggle/working'               if KAGGLE else 'result/results_LSTMAE_Spirit'
...
# Spirit sliding window params — Based on [Bekkouche2025_Spirit]
WINDOW_SIZE = 20
STEP_SIZE   = 10

# Safety cap — set to e.g. 3_000_000 if OOM occurs on Spirit
NROWS_LIMIT = None  # None = full dataset

CKPT = pathlib.Path(BASE_OUT) / f'ckpt_12_lstm_ae_{DS_KEY}.json'
def save_ckpt(d):
...
print(f"✅ Env: {'Kaggle' if KAGGLE else 'Local'} | Device: {DEVICE} | Spirit BiLSTM-AE Standalone")

# ─────────────────────────────────────────────────────────────────────────────
# CELL 2 — Build Spirit Sessions (Sliding Window) Inline
# Based on [Bekkouche2025_Spirit]: window size significantly impacts results
# ─────────────────────────────────────────────────────────────────────────────
if 'sessions_ready' not in ckpt:
    print(f"\n[CELL 2] Building Spirit sessions (WINDOW={WINDOW_SIZE}, STEP={STEP_SIZE}) ...")
    t0 = time.time()
    ...
    vocab_sp = {'<PAD>': 0, '<UNK>': 1}
    for idx, t in enumerate(unique_t):
        vocab_sp[t] = idx + 2
    joblib.dump(vocab_sp, f'{MODEL_DIR}/vocab_spirit_lstm_ae.pkl')
    print(f"  Vocabulary: {len(vocab_sp)} templates (train-only, no leakage)")

    event_ids = np.array(
        [vocab_sp.get(t, 1) for t in all_templates], dtype=np.int32
    )
    ...
    print(f"  ✅ Spirit sessions saved ({elapsed:.0f}s)")
else:
    print("[CELL 2] ⏭️  Sessions already built (checkpoint)")

# Load sessions
tr = np.load(f'{MODEL_DIR}/spirit_sessions_train_lstmae.npz')
va = np.load(f'{MODEL_DIR}/spirit_sessions_val_lstmae.npz')
te = np.load(f'{MODEL_DIR}/spirit_sessions_test_lstmae.npz')
vocab_sp = joblib.load(f'{MODEL_DIR}/vocab_spirit_lstm_ae.pkl')
VS = len(vocab_sp)
...
# Based on [Bekkouche2025_BiLSTM]: BiLSTM-AE captures bidirectional context
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 4 — F1-Sensitive Threshold + Error Computation
# ─────────────────────────────────────────────────────────────────────────────
...
if 'ae_done' not in ckpt:
    print(f"\n[CELL 5] BiLSTM-AE Optimization — Spirit")
    ...
    print("  🔍 Optuna (12 trials) ...")
    study.optimize(objective, n_trials=12, timeout=600)
    bp = study.best_params
    print(f"  🏆 {bp} → Val F1={study.best_value:.4f}")

    # Full Training
    print(f"\n  🚀 Full training (60 epochs, patience=10) ...")
    ...
    plt.title(f'BiLSTM-AE Training Loss — Spirit')
    ...
    ax.set_title('Reconstruction Error Distribution — Spirit BiLSTM-AE')
    ...
    plt.title('ROC — Spirit BiLSTM-AE'); plt.legend(); plt.grid(alpha=0.3)
    plt.tight_layout()
    ...
    ax.set_title('CM — Spirit BiLSTM-AE (Optimized)')
    plt.tight_layout()
    ...
    print(f"\n  ✅ Spirit BiLSTM-AE done ({inf_time:.0f}s inference)")
else:
    print("[CELL 5] ⏭️  BiLSTM-AE already done (checkpoint)")

# ─────────────────────────────────────────────────────────────────────────────
# CELL 6 — VerificationBlock
# ─────────────────────────────────────────────────────────────────────────────
print(f"\n{'='*60}")
print("  ✅ BiLSTM-AE SPIRIT STANDALONE — COMPLETE")
print(f"{'='*60}")
expected_files = [
    (MODEL_DIR, f'lstm_ae_{DS_KEY}_opt.pt'),
    (MODEL_DIR, f'lstm_ae_{DS_KEY}_config.json'),
    (MODEL_DIR, 'vocab_spirit_lstm_ae.pkl'),
    (MODEL_DIR, 'spirit_sessions_train_lstmae.npz'),
    (MODEL_DIR, 'spirit_sessions_val_lstmae.npz'),
    (MODEL_DIR, 'spirit_sessions_test_lstmae.npz'),
