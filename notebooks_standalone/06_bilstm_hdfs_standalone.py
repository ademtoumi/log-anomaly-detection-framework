# =============================================================================
# STANDALONE NOTEBOOK 06 — HDFS Attention-BiLSTM (Fully Independent)
#
# ✅ ZERO dependencies — reads HDFS_Drain.csv directly from
#    /kaggle/input/pfe-log-anomaly/ and builds all sessions inline.
# ✅ One dataset only (HDFS) — RAM stays safe on Kaggle GPU.
# ✅ No output files from any other notebook are required.
#
# Architecture: Embedding → BiLSTM → Attention → Dropout → FC → Sigmoid
...
# ── Fixed seeds everywhere — reproducibility [Bekkouche2025_BiLSTM] ──────────
SEED = 42
random.seed(SEED)
np.random.seed(SEED)
...
BASE_OUT  = '/kaggle/working'               if KAGGLE else 'result/results_AttentionBiLSTM_HDFS'
...
print(f"✅ Device: {DEVICE} | {'Kaggle' if KAGGLE else 'Local'} environment")
...
if 'sessions_ready' not in ckpt:
    ...
    # ── Phase 1: Chunked session aggregation ─────────────────────────────────
    # block_events: BlockId → list of template strings (in arrival order)
    # block_labels: BlockId → 1 if ANY log line for that block is anomalous
    ...
    for chunk in pd.read_csv(filepath, chunksize=500_000,
        ...
        # Anomaly label — 'Label' column with 'Normal' vs anything else
        ...
        for _, row in chunk[['_bid', 'template', '_anom']].iterrows():
            ...
            # Session label = OR across all its lines (any anomaly → anomaly)
    ...
    # ── Phase 2: Build vocabulary from TRAIN sessions only — no leakage ──────
    ...
    # ── Phase 3: Encode sessions as padded int32 sequences ───────────────────
    ...
    # Free the large dicts now — we only need sequences & labels_arr
    del block_events, block_labels
    gc.collect()
    print(f"  Sequences shape: {sequences.shape} | dtype: {sequences.dtype}")

    # ── Phase 4: Temporal split 60 / 20 / 20 on blocks ──────────────────────
    ...
    print(f"     → {MODEL_DIR}/hdfs_sessions_{{train,val,test}}.npz")
    print(f"     → {MODEL_DIR}/vocab_hdfs_opt.pkl")
else:
    print("[CELL 2] ⏭️  Sessions already built (checkpoint 'sessions_ready')")

# ─────────────────────────────────────────────────────────────────────────────
# CELL 3 — Attention-BiLSTM Architecture
# ─────────────────────────────────────────────────────────────────────────────
...
class AttentionBiLSTM(nn.Module):
    ...
    References:
        [Zhang2019_LogRobust] — Attention mechanism for log anomaly detection.
        [Guo2021_LogBERT]     — Bidirectional context is best for offline analysis.
    """
    ...
    def __init__(self, vocab_size, embed_dim=64, hidden_size=128,
        ...
        # Attention: scalar score per timestep → softmax → weighted sum
    ...
    def forward(self, x):
        ...
        # Attention over all T timesteps — [Zhang2019_LogRobust]
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 5 — Optuna Hyperparameter Search + Full Training
# ─────────────────────────────────────────────────────────────────────────────
if 'done' in ckpt:
    print("⏭️  [CELL 5] BiLSTM training already done (checkpoint 'done')")
else:
    print(f"\n{'='*65}")
    print("  🧠 HDFS ATTENTION-BiLSTM — OPTUNA + FULL TRAINING")
    print(f"{'='*65}")

    t0_total = time.time()

    # ── Load sessions saved in CELL 2 ────────────────────────────────────────
    ...
    # ── Optuna: 20 trials, timeout=1200s ──────────────────────────────────────
    ...
    #   [Du2017_DeepLog]: lr≈5e-4, batch=256
    ...
    # Warm-start trial from paper defaults — [Zhang2019_LogRobust] + [Du2017_DeepLog]
    study.enqueue_trial({
        'embed_dim': 64, 'hidden_size': 128, 'num_layers': 2,
        'dropout': 0.3, 'lr': 0.0005, 'batch_size': 256,
    })

    print(f"\n  🔍 Optuna search (20 trials, timeout=1200s) ...")
    ...
    print(f"\n  🚀 Full training (max_epochs=50, patience=12) ...")
    ...
    # ── Final model reload + TEST evaluation (test set touched exactly once) ──
    model = AttentionBiLSTM(
        vocab_size   = VOCAB_SIZE,
    ...
    print(f"\n  ┌─────────────────────────────────────────────────────┐")
    print(f"  │  RESULTS vs PAPER — HDFS Attention-BiLSTM           │")
    print(f"  │  Paper [Bekkouche2025_BiLSTM] F1 : {paper_f1:.4f}          │")
    print(f"  │  Our F1                        : {our_f1:.4f}          │")
    print(f"  │  Delta                         : {delta:+.4f}          │")
    print(f"  └─────────────────────────────────────────────────────┘")

    print(f"\n  📊 TEST RESULTS — HDFS Attention-BiLSTM:")
    ...
    print(f"  ✅ Model saved → {MODEL_DIR}/bilstm_hdfs_opt.pt")
    ...
    print(f"  ✅ Config saved → {MODEL_DIR}/bilstm_hdfs_config.json")

    pd.DataFrame([metrics]).round(4).to_csv(
        f'{REPORT}/bilstm_hdfs_results.csv', index=False)
    print(f"  ✅ Results saved → {REPORT}/bilstm_hdfs_results.csv")
    ...
    ax1.set_title('Training Loss — HDFS Attention-BiLSTM', fontweight='bold')
    ...
    ax2.set_title('Validation F1 (optimal threshold) — HDFS Attention-BiLSTM',
                  fontweight='bold')
    ...
    plt.close()
    print(f"  ✅ Curves → {REPORT}/bilstm_hdfs_curves.png")
    ...
    plt.title('ROC Curve — HDFS Attention-BiLSTM', fontweight='bold')
    plt.legend(loc='lower right'); plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(f'{REPORT}/bilstm_hdfs_roc.png', dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  ✅ ROC    → {REPORT}/bilstm_hdfs_roc.png")
    ...
    ax.set_title('Confusion Matrix — HDFS Attention-BiLSTM', fontweight='bold')
    plt.tight_layout()
    plt.savefig(f'{REPORT}/bilstm_hdfs_cm.png', dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  ✅ CM     → {REPORT}/bilstm_hdfs_cm.png")
    ...
    print(f"\n  ✅ HDFS BiLSTM complete — total wall time: {total_time:.0f}s "
...
# ─────────────────────────────────────────────────────────────────────────────
print(f"\n{'='*65}")
print("  ✅  STANDALONE NOTEBOOK 06 — HDFS ATTENTION-BiLSTM — COMPLETE")
print(f"{'='*65}")
...
print(f"  Status: {'🎉 All outputs present' if all_ok else '⚠️  Some outputs missing'}")
print(f"\n  Model dir  : {MODEL_DIR}")
print(f"  Report dir : {REPORT}")
print(f"\n  Paper citations in this notebook:")
print(f"    [Zhang2019_LogRobust]  — Attention over ALL hidden states > last hidden state")
...
print(f"    [Bekkouche2025_BiLSTM] — F1-optimal threshold on VAL, pos_weight strategy")
print(f"\n  KEY CHANGE vs original:")
print(f"    ❌ Old: fixed threshold=0.5 for both val early-stopping AND test evaluation")
