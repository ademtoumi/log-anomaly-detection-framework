# =============================================================================
# STANDALONE NOTEBOOK — Random Forest on BGL (Fully Independent)
#
...
BASE_OUT = '/kaggle/working'               if KAGGLE else 'result/results_RF_BGL'
...
if 'data_ready' not in ckpt:
    ...
    # Load only the columns we need → saves ~50% RAM
    ...
    print("[CELL 2] ⏭️  Loading from saved TF-IDF ...")
...
if 'rf_done' not in ckpt:
    ...
    print(f"\n  📊 TEST RESULTS — BGL Random Forest:")
    ...
    ax.set_title('RF CM — BGL (Optimized)', fontweight='bold')
    plt.tight_layout()
    ...
    plt.title('RF ROC — BGL (Optimized)', fontweight='bold')
    ...
    ax.set_title(f'RF Feature Importance — BGL Top-{top_n}', fontweight='bold')
    ...
    try:
        ...
        plt.title('SHAP Summary — RF BGL (Anomaly class)', fontweight='bold')
        plt.tight_layout()
    ...
    print(f"\n  ✅ RF BGL done in {time.time()-t0:.0f}s")
else:
...
# ─────────────────────────────────────────────────────────────────────────────
print(f"\n{'='*60}")
print("  ✅ RF BGL STANDALONE — COMPLETE")
print(f"{'='*60}")
...
for fname in ['rf_bgl_results.csv', 'rf_cm_bgl.png',
    ...
    p = f'{REPORT}/{fname}'
    print(f"  {'✅' if os.path.exists(p) else '❌'} {fname}")
print(f"  📊 Results → {REPORT}/rf_bgl_results.csv")
