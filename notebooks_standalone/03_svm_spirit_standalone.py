# =============================================================================
# STANDALONE NOTEBOOK — SVM on Spirit (Fully Independent)
#
# Based on [Bekkouche2025_Spirit] — SVM + Word2Vec achieves F1=0.966 on Spirit.
...
# Based on [Ribeiro2016_LIME] and [Lundberg2017_SHAP] — local (LIME) and global (SHAP)
#   explainability integrated for anomaly diagnostics.
#
# ✅ ZERO dependencies — reads raw Spirit_Drain.csv directly.
# ✅ Builds its own Word2Vec embeddings inline then frees raw logs.
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 2 — Load Spirit & Train Word2Vec inline
...
if 'data_ready' not in ckpt:
    ...
    print(f"  ✅ Word2Vec vectorization done ({time.time()-t0:.0f}s)")
    ckpt['data_ready'] = True; save_ckpt(ckpt)

else:
    print("[CELL 2] ⏭️  Loading from saved Word2Vec model and vectors ...")
...
if 'svm_done' not in ckpt:
    ...
    ax.set_title('SVM CM — Spirit (Word2Vec)', fontweight='bold')
    plt.tight_layout()
    ...
    plt.title('SVM ROC — Spirit (Word2Vec)'); plt.legend(); plt.grid(alpha=0.3)
    plt.tight_layout(); plt.savefig(f'{REPORT}/svm_roc_spirit.png', dpi=300); plt.close()

    # ── Explainability Section [Ribeiro2016_LIME, Lundberg2017_SHAP] ──────────
    try:
        ...
        plt.title("SHAP Summary — SVM Spirit (Word2Vec)", fontweight="bold")
        plt.tight_layout()
    ...
    try:
        ...
        if len(anom_indices) > 0:
            ...
            plt.close()
            print("  ✅ LIME local explanation saved.")
    except Exception as e:
    ...
    print(f"\n  ✅ SVM Spirit done in {time.time()-t0:.0f}s")
else:
    print("[CELL 3] ⏭️  SVM already done (checkpoint)")

# ─────────────────────────────────────────────────────────────────────────────
# CELL 4 — Verification
# ─────────────────────────────────────────────────────────────────────────────
print(f"\n{'='*60}")
print("  ✅ SVM SPIRIT STANDALONE — COMPLETE")
print(f"{'='*60}")
...
print(f"  📊 Results → {REPORT}/svm_spirit_results.csv")
