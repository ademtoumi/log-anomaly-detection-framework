...
# =============================================================================
# LOAD ALL 3 CACHES — exact paths from Kaggle dataset structure
# =============================================================================

# ── features.pkl from hierattn_output ────────────────────────────────────────
CACHE1 = '/kaggle/input/hierattn-output/hierattn_output/cache'
with open(os.path.join(CACHE1, 'features.pkl'), 'rb') as f:
    C = pickle.load(f)

feat_train=C['feat_train']; feat_val=C['feat_val']; feat_test=C['feat_test']
y_train=C['y_train']; y_val=C['y_val']; y_test=C['y_test']
X_train_struct=C['X_train_struct']
X_val_struct=C['X_val_struct']
...
# ── baseline_results.pkl from hierattn_output1 ───────────────────────────────
CACHE2 = '/kaggle/input/hierattn-output1/hierattn_output/cache'
with open(os.path.join(CACHE2, 'baseline_results.pkl'), 'rb') as f:
    B = pickle.load(f)
...
# ── hierattn_results.pkl from hierattn_output2 ───────────────────────────────
CACHE3 = '/kaggle/input/hierattn-output2/hierattn_output/cache'
with open(os.path.join(CACHE3, 'hierattn_results.pkl'), 'rb') as f:
    H = pickle.load(f)
...
# Reconstruct preds for baselines (needed for confusion matrices)
dl_preds = (dl_scores >= 0.5).astype(int)   # fallback threshold

# Tune DeepLog threshold from saved scores
val_dl_scores = dl_scores[:len(y_val)]       # not available — use 0.5 as fallback
best_dl_thresh, best_dl_f1 = 0.0, 0.0
# Use stored precision/recall to back-derive threshold — skip, use stored results directly

lb_preds_test = (lb_probs_test >= 0.5).astype(int)
...
def train_variant(model, use_aux=True):
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        with torch.no_grad():
            for batch in dl_v:
                ev,pf,st,sf,am,labels,repl = [b.to(DEVICE) for b in batch]
                preds.extend(model(ev,pf,st,sf,am,return_aux=False)[0].argmax(-1).cpu().numpy())
        vf1 = f1_score(y_val, preds, zero_division=0)
        if vf1 > best_f1:
            best_f1=vf1; best_st={k:v.cpu().clone() for k,v in model.state_dict().items()}; pat=0
        else: pat += 1
...
def eval_variant(model):
    ...
    with torch.no_grad():
        for batch in dl_tst:
            ...
            probs.extend(p.tolist()); preds.extend((p>=0.5).astype(int).tolist())
...
print(f'        {res_seq}')
try:
    f_,t_,_=roc_curve(y_test,probs_seq); all_roc['Seq Only']=(f_,t_,res_seq['AUC'])
except: pass
...
print(f'        {res_struct}')
try:
    f_,t_,_=roc_curve(y_test,probs_struct); all_roc['Struct Only']=(f_,t_,res_struct['AUC'])
except: pass
...
print(f'        {res_noaux}')
try:
    f_,t_,_=roc_curve(y_test,probs_noaux); all_roc['No Aux Head']=(f_,t_,res_noaux['AUC'])
except: pass
...
for i, name in enumerate(['DeepLog','LogBERT','Seq Only','Struct Only','No Aux Head','HierAttn-Block']):
    if name not in all_roc: continue
    fpr_,tpr_,auc_ = all_roc[name]
    ax2.plot(fpr_,tpr_,color=PALETTE[i%len(PALETTE)],lw=2,label=f'{name}  (AUC={auc_:.4f})')
...
# Confusion matrices baselines — rebuild preds from stored probs/scores
dl_preds_final = (dl_scores >= 0.5).astype(int)
lb_preds_final = (lb_probs_test >= 0.5).astype(int)
for bname, bpreds, cmap in [('DeepLog',dl_preds_final,'Blues'),('LogBERT',lb_preds_final,'Oranges')]:
    fig_,ax_=plt.subplots(figsize=(5,4))
    sns.heatmap(confusion_matrix(y_test,bpreds),annot=True,fmt='d',cmap=cmap,
...
MODEL_PATH = '/kaggle/input/hierattn-output2/hierattn_output/models/hierattn_best.pt'
...
for k,(stype,sidx) in enumerate(selected):
    ...
    axes4[k].set_xticklabels([str(feat_sel['event_ids'][i]) for i in range(n_real)],
...
feat_names = [f'Template_{i+1}_count' for i in range(5)] + \
             ['size_std','n_unique_ips','session_duration','max_gap','missing_allocate','repl_neq3']
print('  Computing permutation importance ...')
rf_pi = RandomForestClassifier(n_estimators=100, random_state=SEED, n_jobs=-1)
rf_pi.fit(X_train_struct, y_train)
pi   = permutation_importance(rf_pi,X_test_struct,y_test,n_repeats=10,random_state=SEED,scoring='f1')
sidx = np.argsort(pi.importances_mean)[::-1]
fig5,ax5=plt.subplots(figsize=(9,5))
ax5.barh([feat_names[i] for i in sidx[::-1]], pi.importances_mean[sidx[::-1]],
         xerr=pi.importances_std[sidx[::-1]],
         color=PALETTE[0],ecolor='#999999',capsize=4,height=0.6)
ax5.set_xlabel('Mean Decrease in F1 (Permutation Importance)',fontsize=11)
ax5.set_title('Structural Feature Importance — Permutation Method',fontsize=13,fontweight='bold')
...
len_normal  = [int(feat_test[i]['attention_mask'].sum()) for i in range(len(feat_test)) if feat_test[i]['label']==0]
len_anomaly = [int(feat_test[i]['attention_mask'].sum()) for i in range(len(feat_test)) if feat_test[i]['label']==1]
...
ax6.set_title('Session Length Distribution — Normal vs. Anomaly',fontsize=13,fontweight='bold')
...
print(f"  {'DeepLog':<22} {deepl['Precision']:>10} {deepl['Recall']:>8} {deepl['F1']:>8} {deepl['AUC']:>8}")
print(f"  {'LogBERT':<22} {logb['Precision']:>10} {logb['Recall']:>8} {logb['F1']:>8} {logb['AUC']:>8}")
print(f"\n  Improvement over DeepLog : +{(hier['F1']-deepl['F1'])*100:.2f}% F1")
print(f"  Improvement over LogBERT : +{(hier['F1']-logb['F1'])*100:.2f}% F1")
print('='*65)

# Save final JSON
results_all = {
    'HierAttnBlock': hier, 'DeepLog': deepl, 'LogBERT': logb,
    'Ablation': {
        'Sequence Only':         res_seq,
        'Structural Only':       res_struct,
        'No Auxiliary Head':     res_noaux,
...
print('\nFIGURES:')
for fname in ['fig1_training_curves.png','fig2_roc_curves.png',
              'fig3_confusion_matrix.png','fig4_attention_heatmap.png',
              'fig5_feature_importance.png','fig6_session_length_distribution.png',
              'cm_deeplog.png','cm_logbert.png']:
