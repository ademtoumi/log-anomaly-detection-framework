# =============================================================================
# STANDALONE NOTEBOOK 15 — BiLSTM Autoencoder on BGL (Fully Independent)
#
...
#   [Bekkouche2024]          — MSE reconstruction error for unsupervised AE scoring.
...
BASE_OUT = '/kaggle/working'               if KAGGLE else 'result/results_BiLSTMAE_BGL'
...
print(f"✅ Env: {'Kaggle' if KAGGLE else 'Local'} | Device: {DEVICE} | BGL BiLSTM-AE Standalone")
...
if 'ae_done' not in ckpt:
    print(f"\n[CELL 5] BiLSTM-AE Optimization — BGL")
    ...
    plt.title(f'BiLSTM-AE Training Loss — BGL')
    ...
    ax.set_title('Reconstruction Error Distribution — BGL BiLSTM-AE')
    ...
    plt.title('ROC — BGL BiLSTM-AE'); plt.legend(); plt.grid(alpha=0.3)
    plt.tight_layout()
    ...
    ax.set_title('CM — BGL BiLSTM-AE (Optimized)')
    plt.tight_layout()
    ...
    print(f"\n  ✅ BGL BiLSTM-AE done ({inf_time:.0f}s inference)")
else:
...
# ─────────────────────────────────────────────────────────────────────────────
print(f"\n{'='*60}")
print("  ✅ BiLSTM-AE BGL STANDALONE — COMPLETE")
print(f"{'='*60}")
expected_files = [
