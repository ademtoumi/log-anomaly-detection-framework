# =============================================================================
# STANDALONE NOTEBOOK 17 — DeepLog on BGL (Fully Independent)
#
# ✅ ZERO dependencies — reads raw BGL_Drain.csv directly from Kaggle input.
# ✅ Builds BGL sessions inline (sliding window, vocab, int32 sequences).
# ✅ One dataset only (BGL) — RAM stays safe on Kaggle T4/P100.
# ✅ Trains on NORMAL sessions only — next-key prediction for anomaly scoring.
#
# References:
#   [Du2017_DeepLog]         — DeepLog next-key prediction using LSTM on normal logs.
#   [Bekkouche2024]          — Unsupervised next-key prediction thresholding on BGL.
#   [Zhang2019_LogRobust]    — Sequential temporal split is scientifically honest.
#
# Architecture:
#   Embedding(vocab_size, embed_dim, padding_idx=0)
#   → LSTM(embed_dim, hidden_size, num_layers)
...
print(f"✅ Env: {'Kaggle' if KAGGLE else 'Local'} | Device: {DEVICE} | BGL DeepLog Standalone")

# ─────────────────────────────────────────────────────────────────────────────
# CELL 2 — Build BGL Sessions (Sliding Window) Inline
# ─────────────────────────────────────────────────────────────────────────────
if 'sessions_ready' not in ckpt:
    ...
    print(f"  ✅ BGL sessions saved ({elapsed:.0f}s)")
else:
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 3 — DeepLog Predictor Architecture
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 4 — DeepLog Anomaly Evaluation
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 5 — Training DeepLog (Unsupervised on Normal windows)
# ─────────────────────────────────────────────────────────────────────────────
if 'deeplog_done' not in ckpt:
    print(f"\n[CELL 5] DeepLog Training — BGL")
    ...
    print("\n  🔍 Tuning top-k parameter on validation set ...")
    best_f1, best_k = 0, 1
    ...
    print(f"  🏆 Best k: {best_k} (Val F1: {best_f1:.4f})")
    ...
    plt.title(f'DeepLog Training Loss — BGL')
    ...
    ax.set_title(f'CM — BGL DeepLog (k={best_k})')
    plt.tight_layout()
    ...
    print(f"\n  ✅ BGL DeepLog done ({inf_time:.0f}s inference)")
else:
    print("[CELL 5] ⏭️  DeepLog already done (checkpoint)")

# ─────────────────────────────────────────────────────────────────────────────
# CELL 6 — VerificationBlock
# ─────────────────────────────────────────────────────────────────────────────
print(f"\n{'='*60}")
print("  ✅ DEEPLOG BGL STANDALONE — COMPLETE")
print(f"{'='*60}")
expected_files = [
