# =============================================================================
# STANDALONE NOTEBOOK — Random Forest on Spirit (Fully Independent)
#
# Based on [Bekkouche2025_Spirit] — RF + TF-IDF achieves F1=0.962 on Spirit.
#   class_weight='balanced_subsample' effective for imbalanced log datasets.
# Based on [Lundberg2017_SHAP] — TreeSHAP provides exact feature attributions
...
# ✅ Chunked CSV reading — Spirit is large (~10M+ rows).
#
# Kaggle setup:
...
BASE_OUT = '/kaggle/working'               if KAGGLE else 'result/results_RF_Spirit'
...
# Safety cap for Spirit — reduce to e.g. 3_000_000 if Kaggle OOM (16 GB RAM).
# Set to None to use the full dataset (recommended for final submission).
# Example: NROWS_LIMIT = 3_000_000
...
print(f"{'Kaggle' if KAGGLE else 'Local'} | Spirit Random Forest Standalone")

# ─────────────────────────────────────────────────────────────────────────────
# CELL 2 — Load Spirit (chunked) + Build TF-IDF Splits Inline
# Based on [Bekkouche2025_Spirit]: larger vocab (10k) improves tree/SVM models
...
if 'data_ready' not in ckpt:
    ...
    # Chunked load — Spirit is large; collect only what we need
    all_templates = []
    all_labels    = []
    rows_loaded   = 0
    ...
    print("[CELL 2] ⏭️  Re-loading from saved TF-IDF vectorizer ...")
    vectorizer = joblib.load(f'{BASE_OUT}/models/tfidf_{DS_KEY}_opt.pkl')
    filepath = find_file('Spirit_Drain.csv')
    all_templates, all_labels, rows_loaded = [], [], 0
    for chunk in pd.read_csv(filepath, chunksize=500_000,
                              usecols=['template', 'label'],
                              on_bad_lines='skip', low_memory=False):
        all_templates.extend(chunk['template'].fillna('').tolist())
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 3 — Optuna + Random Forest Training
...
if 'rf_done' not in ckpt:
    print(f"\n[CELL 3] RF Optuna optimization on Spirit ...")
    t0 = time.time()
    ...
    print(f"  🏆 Best: {best} → Val F1={study.best_value:.4f}")
    ...
    roc_auc = auc(fpr, tpr)
    metrics = {
        'Dataset': 'SPIRIT', 'Model': 'RandomForest', 'Type': 'Supervised (ML)',
    ...
    print(f"\n  📊 TEST RESULTS — Spirit Random Forest:")
    ...
    pd.DataFrame([metrics]).round(4).to_csv(f'{REPORT}/rf_spirit_results.csv', index=False)
    ...
    ax.set_title('RF CM — Spirit (Optimized)', fontweight='bold')
    plt.tight_layout()
    plt.savefig(f'{REPORT}/rf_cm_spirit.png', dpi=300); plt.show()
    ...
    plt.title('RF ROC — Spirit (Optimized)', fontweight='bold')
    plt.legend(loc='lower right'); plt.grid(alpha=0.3); plt.tight_layout()
    plt.savefig(f'{REPORT}/rf_roc_spirit.png', dpi=300); plt.show()
    ...
    ax.set_title(f'RF Feature Importance — Spirit Top-{top_n}', fontweight='bold')
    ax.set_ylabel('Gini Importance')
    plt.tight_layout()
    plt.savefig(f'{REPORT}/rf_feat_importance_spirit.png', dpi=300); plt.show()

    # ── SHAP TreeExplainer [Lundberg2017_SHAP] ────────────────────────────────
    # TreeSHAP: exact Shapley values for tree ensembles, O(TLD) complexity.
    # Wrapped in try/except — shap may be unavailable in some Kaggle environments.
    try:
        import shap
        ...
        plt.title('SHAP Summary — RF Spirit (Anomaly class)', fontweight='bold')
        plt.tight_layout()
        plt.savefig(f'{REPORT}/rf_shap_summary_spirit.png', dpi=300, bbox_inches='tight')
        plt.show()
        print("  ✅ SHAP done")
    ...
    print(f"\n  ✅ RF Spirit done in {time.time()-t0:.0f}s")
else:
    print("[CELL 3] ⏭️  RF already done (checkpoint)")

# ─────────────────────────────────────────────────────────────────────────────
# CELL 4 — Verification
# ─────────────────────────────────────────────────────────────────────────────
print(f"\n{'='*60}")
print("  ✅ RF SPIRIT STANDALONE — COMPLETE")
print(f"{'='*60}")
...
for fname in ['rf_spirit_results.csv', 'rf_cm_spirit.png',
              'rf_roc_spirit.png', 'rf_feat_importance_spirit.png']:
    p = f'{REPORT}/{fname}'
    print(f"  {'✅' if os.path.exists(p) else '❌'} {fname}")
print(f"  📊 Results → {REPORT}/rf_spirit_results.csv")
