# =============================================================================
# STANDALONE NOTEBOOK 12c — Optimized BiLSTM Autoencoder — HDFS Log Anomaly
#                           (Fully Independent, Zero Dependencies)
#
# ✅ ZERO dependencies — reads HDFS_Drain.csv directly.
# ✅ Builds HDFS sessions inline (BlockId grouping, stratified split).
...
# ✅ AttentionPool: hybrid context = (BiLSTM hidden state + attention over enc_out)
#
# Architecture (vs NB12 / NB12b):
#   Embedding → BiLSTM Encoder → AttentionPool + combine → LSTM Decoder → MSE
...
#   Reduces LR when val F1 stagnates — adaptive LR for better convergence.
#
# References:
#   [Bekkouche2025_BiLSTM] — BiLSTM-AE, F1=0.993 on HDFS; paper baseline.
#   [Du2017_DeepLog]        — HDFS sessions naturally grouped by BlockId.
#   [Zhang2019_LogRobust]   — Attention over hidden states improves log detection.
#   [Guo2021_LogBERT]       — Bidirectional context for offline log analysis.
#
# Kaggle setup:
...
#
# Paper target [Bekkouche2025_BiLSTM]: F1=0.9930  → Delta = -0.0359
#   (Unsupervised AE gap vs supervised BiLSTM — expected & scientifically valid)
# =============================================================================
...
BASE_OUT = '/kaggle/working' if KAGGLE else 'result/results_BiLSTMAE_HDFS_v3'
REPORT   = f'{BASE_OUT}/pfe_report'
os.makedirs(REPORT, exist_ok=True)
print(f"{'Kaggle' if KAGGLE else 'Local'} environment | Outputs → {REPORT}")

# ─────────────────────────────────────────────────────────────────────────────
# CELL 1 — Locate HDFS_Drain.csv
# ─────────────────────────────────────────────────────────────────────────────
csv_candidates = [
...
if not csv_path:
    raise FileNotFoundError("HDFS_Drain.csv not found — check Kaggle dataset attachment")

# ─────────────────────────────────────────────────────────────────────────────
# CELL 2 — Load & Group Sessions (Chunked, RAM-safe)
...
block_labels = {}   # BlockId → int (max anomaly flag)
...
for chunk in pd.read_csv(csv_path, chunksize=500_000,
    ...
    # Anomaly label: any anomalous line → session is anomalous
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 3 — Stratified Split 90 / 10 / 10
# [Bekkouche2025_BiLSTM]: stratified split preserves anomaly ratio.
# ─────────────────────────────────────────────────────────────────────────────
print("\nApplying Stratified Split — 90/10 train/test, then 90/10 train/val")
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 4 — Vocabulary (train-only — no leakage)
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 5 — Model Hyperparameters
# ─────────────────────────────────────────────────────────────────────────────
# Best params from result-6 Kaggle run (31 epochs, ReduceLROnPlateau).
# No Optuna needed here — fixed best config for reproducibility.
PARAMS = {
    'embed_dim'  : 64,
    'hidden_size': 256,
    'num_layers' : 2,
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 6 — BiLSTM Autoencoder with AttentionPool
...
class AttentionPool(nn.Module):
    """Soft attention over encoder output — masks padding before softmax."""
...
class BiLSTMAutoencoder(nn.Module):
    ...
    Encoder: Bidirectional LSTM → dual context vector
    ...
    References:
        [Bekkouche2025_BiLSTM] — BiLSTM-AE baseline, F1=0.993 on HDFS.
        [Zhang2019_LogRobust]  — Attention over ALL hidden states.
        [Guo2021_LogBERT]      — Bidirectional context for offline log analysis.
    """
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 7 — Scoring & Threshold Search
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 8 — Training (Normal-only, ReduceLROnPlateau)
# ─────────────────────────────────────────────────────────────────────────────
# Train exclusively on normal sessions — reconstruction error separates anomalies.
...
# ReduceLROnPlateau on val F1 — reduces LR when F1 stagnates
...
print("\nTraining BiLSTM-AE (Optimized — AttentionPool + ReduceLROnPlateau)...")
for epoch in range(1, MAX_EPOCHS + 1):
    ...
    # Validation: compute scores → F1-optimal threshold
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 9 — Final Test Evaluation (test set touched exactly once)
# ─────────────────────────────────────────────────────────────────────────────
...
print("\n" + "=" * 57)
print("  FINAL TEST RESULTS — BiLSTM-AE HDFS (Optimized)")
print("=" * 57)
...
print(f"Delta     : {delta:+.4f}  (unsupervised vs supervised — expected gap)")
print("=" * 57)
...
ax.set_title('BiLSTM-AE (Optimized) Confusion Matrix — HDFS', fontweight='bold')
...
ax.set_title('ROC Curve — BiLSTM-AE (Optimized) HDFS', fontweight='bold')
...
ax1.set_title('Training Loss — BiLSTM-AE Optimized HDFS')
...
ax2.set_title('Validation F1 (optimal threshold) — HDFS')
...
ax.set_title('Anomaly Score Distribution — BiLSTM-AE (Optimized) HDFS',
             fontweight='bold')
ax.set_xlabel('Score (0.5·mean + 0.5·max MSE)')
...
# ─────────────────────────────────────────────────────────────────────────────
print("\n" + "=" * 57)
print("  NOTEBOOK 12c — BiLSTM-AE OPTIMIZED HDFS — COMPLETE")
print("=" * 57)
...
print(f"    ✅ Dual context: hidden state + AttentionPool (fused bottleneck)")
print(f"    ✅ Score: 0.5·mean + 0.5·max masked MSE per session")
print(f"    ✅ Scheduler: ReduceLROnPlateau(mode='max') on val F1")
print(f"    ✅ Split: Stratified 90/10/10 preserves anomaly ratio")
print(f"    ✅ Threshold: F1-optimal on val only (10,000 grid points)")
print(f"\n  Paper citations:")
print(f"    [Bekkouche2025_BiLSTM] — BiLSTM-AE baseline, F1=0.993 HDFS")
print(f"    [Du2017_DeepLog]       — HDFS sessions grouped by BlockId")
print(f"    [Zhang2019_LogRobust]  — Attention over ALL hidden states")
print(f"    [Guo2021_LogBERT]      — Bidirectional context for offline logs")
print("=" * 57)
