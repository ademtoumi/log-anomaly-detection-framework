...
OUTPUT_DIR = '/kaggle/working' if os.path.exists('/kaggle') else 'result/results_DT_BGL'
DS_KEY     = 'bgl'
...
print(f"{'='*60}")
print(f"  Decision Tree – BGL (standalone)")
print(f"{'='*60}")
...
if 'data_ready' not in ckpt:
    ...
    print('[1-2/4] Data already prepared – reloading …')
    ...
    X_test  = tfidf.transform(X_raw_test).astype(np.float32)
    del X_raw_train, X_raw_val, X_raw_test; gc.collect()
    print(f"   Reloaded – train {X_train.shape}")
...
if 'dt_done' not in ckpt:
    ...
    print('[3/4] DT already trained – loading from checkpoint …')
...
ax.set_title(f'DT BGL – Confusion Matrix\nF1={test_f1:.4f}')
for i in range(2):
    for j in range(2):
...
ax.set_title(f'Top-{top_k} DT Feature Importances – BGL')
plt.tight_layout()
...
try:
    ...
    ax.set_title(f'Top-{top_k} SHAP Feature Importance – DT BGL  [Lundberg2017_SHAP]')
    plt.tight_layout()
...
# =============================================================================
print('\n' + '='*60)
print('CLASSIFICATION REPORT – DT BGL')
print('='*60)
...
if os.path.exists(shap_png):
    print(f"  [✓] {os.path.basename(shap_png)}  ({os.path.getsize(shap_png):,} bytes)")
