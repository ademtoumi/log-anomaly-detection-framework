...
# Spirit Attention-BiLSTM — Standalone Kaggle Notebook
# =============================================================================
# References:
...
OUTPUT_DIR = '/kaggle/working'               if os.path.exists('/kaggle') else 'result/results_AttentionBiLSTM_Spirit'
...
if not ckpt.get('sessions_ready'):
    print("\n[1/4] Building Spirit sliding-window sessions …")

    # ------------------------------------------------------------------
    # 1a. Chunked read — collect templates & labels
    # ------------------------------------------------------------------
    all_templates = []
    all_labels    = []
    rows_read     = 0
    ...
    for chunk in reader:
        ...
        if NROWS_LIMIT and rows_read >= NROWS_LIMIT:
            print(f"  NROWS_LIMIT={NROWS_LIMIT:,} reached — stopping early.")
            break
    ...
    # ------------------------------------------------------------------
    # 1b. Build vocabulary (train split only — no leakage)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # 1c. Convert templates → event IDs
    # ------------------------------------------------------------------
    unk_id     = vocab['<UNK>']
    ...
    print(f"  Building {n_windows:,} windows (W={WINDOW_SIZE}, S={STEP_SIZE}) …")
    ...
    for i in range(n_windows):
        ...
        # window label = max (any anomaly → anomaly) [Bekkouche2025_Spirit]
    ...
    print("[1/4] Sessions already built — skipping.")
...
class AttentionBiLSTM(nn.Module):
    """
    Embedding → BiLSTM → Attention → Dropout → FC(1)
...
def train_model(params: dict, max_epochs: int = 60,
    ...
    Early stopping is on val F1 at the threshold that maximises val F1 — NOT fixed 0.5.
...
# Wrapped in checkpoint guard — skips entirely if 'done' is already set.
# This ensures a Kaggle timeout mid-training can be resumed cleanly.
# =============================================================================
if ckpt.get('done'):
    print("\n[3/4] ⏭️  Optuna + training already done (checkpoint 'done') — skipping.")
else:
    print("\n[3/4] Optuna hyper-parameter search (20 trials, timeout=1200 s) …")
    ...
    # =========================================================================
    print("\n[4/4] Final training with best params (max 60 epochs, patience=12) …")
    t0 = time.time()
    ...
    # Threshold derived from val set — test set touched exactly once.
    # [Zhang2019_LogRobust]: threshold must be set on held-out validation data.
    # =========================================================================
    print("\nEvaluating on test set (threshold from val) …")
    ...
    # Apply val-optimal threshold — never search on test
    ...
    # Paper target: [Bekkouche2025_Spirit] F1 ≈ 0.96 on Spirit
    PAPER_F1 = 0.96
    ...
    fig.suptitle('Spirit — Attention-BiLSTM Training Summary', fontsize=14)
...
print("=" * 60)
print("Notebook 08 — Spirit Attention-BiLSTM complete.")
