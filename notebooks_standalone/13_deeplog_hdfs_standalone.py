# =============================================================================
# STANDALONE NOTEBOOK 13 — DeepLog on HDFS  (Du et al., CCS 2017)
#
# REWRITTEN VERSION — all fixes applied for maximum F1
#
# Reference:
...
#   FIX 5: groupby() instead of iterrows() — ~50× faster session building
...
#   FIX 8: Epochs 20→30 + early stopping on val F1 (patience=5)
...
#   • Window anomalous iff true next-key ∉ top-k predictions
#   • Session anomalous iff ANY window is anomalous
#   • k selected on val set by best F1 on full val (normal+anomaly)
#   • Apply best k to test set ONCE
#
...
BASE_OUT  = '/kaggle/working' if KAGGLE else 'result/results_DeepLog_HDFS_v1'
...
# ── Paper hyper-parameters [Du2017] ───────────────────────────────────────────
# WINDOW_SIZE=10 is paper-default and optimal for HDFS [Du2017 §5.1]
WINDOW_SIZE = 10
HIDDEN_SIZE = 64
EMBED_DIM   = 64
NUM_LAYERS  = 2
DROPOUT     = 0.0
# FIX 8: Increased from 20→30 with early stopping
EPOCHS      = 30
...
print("=" * 70)
print("  DeepLog HDFS — REWRITTEN [Du2017]")
print("=" * 70)
...
# =============================================================================
# CELL 2 — Load Sessions  (FIX 5: groupby, not iterrows)
# =============================================================================
print("\n  Loading sessions ...")
t0 = time.time()

session_templates = defaultdict(list)   # blk → [template_str, ...]
session_labels    = {}                  # blk → 0 or 1
...
for chunk in pd.read_csv(csv_path, chunksize=CHUNK_SIZE,
    ...
    # FIX 5: vectorized groupby (~50× faster than iterrows)
...
# =============================================================================
# CELL 3 — FIX 2+3: Ratio-based stratified split
# =============================================================================
...
# Step 2: from remaining 90%, hold out ~11.1% → 10% of total for val
...
# Training uses normal sessions ONLY (paper: model sees no anomalies) [Du2017 §5.2]
...
#   Unseen templates at test time → 0 (PAD sentinel)
...
print(f"  Train-vocab  : {len(vocab)} templates  (+ PAD=0 → VOCAB_SIZE={VOCAB_SIZE})")
...
for b in test_blocks:
    for t in session_templates[b]:
        if t not in vocab:
            unseen_test.add(t)
print(f"  Unseen in test: {len(unseen_test)} templates → flagged anomalous directly")


def encode_session(blk):
    """
    Encode session as int IDs.
    Unseen template → 0 (PAD sentinel, flagged anomalous before model).
    """
    return [vocab.get(t, 0) for t in session_templates[blk]]

# =============================================================================
# CELL 5 — Sliding Windows
# =============================================================================
...
# Train windows: normal sessions only [Du2017 §5.2]
...
# =============================================================================
# CELL 6 — Model (paper-exact architecture) [Du2017 §5.1]
# =============================================================================
...
class DeepLog(nn.Module):
    ...
    Embedding → 2-layer LSTM → Linear(hidden → vocab_size)
    """
...
# =============================================================================
# CELL 7 — Scoring Function (needed during training for val F1)
#
...
def score_sessions_topk(blocks, k_values, batch_size=8192):
    """
    Returns dict: k → np.array(len(blocks),) of 0/1 anomaly flags.

    Two-signal detection protocol:
      1. Any session containing an unseen template (PAD=0) → immediately anomalous (100% anomaly).
      2. Any session with only known templates and len >= WINDOW_SIZE + 1 → run LSTM top-k check.
      3. Any session with only known templates and len < WINDOW_SIZE + 1 → normal.
    """
    model.eval()
...
# =============================================================================
# CELL 8 — Training with Early Stopping & LR Scheduler
#
...
# FIX 9: LR scheduler — reduce on plateau of val loss
...
for epoch in range(1, EPOCHS + 1):
    ...
    for X_batch, y_batch in train_dl:
        ...
        if use_amp:
            ...
            # Gradient clipping [Du2017] — stabilize LSTM training
    ...
    # ── Val loss (on normal val windows — for LR scheduler) ───────────
    model.eval()
...
print(f"  Scoring {len(val_blocks):,} val sessions (ALL — normal + anomaly) ...")
...
for k in K_SEARCH:
    ...
    marker = " ← best" if k_f1 > best_k_f1 else ""
...
# =============================================================================
# CELL 10 — Test Evaluation (touched EXACTLY ONCE)
# =============================================================================
...
print(f"\n  {'='*60}")
print(f"  FINAL TEST RESULTS — DeepLog HDFS [Du2017]")
print(f"  {'='*60}")
...
if f1 >= 0.96:   grade = "✅  EXCELLENT (≥ paper F1)"
elif f1 >= 0.95: grade = "✅  EXCELLENT"
elif f1 >= 0.90: grade = "🟡  GOOD"
elif f1 >= 0.80: grade = "🟠  ACCEPTABLE"
...
print(f"\n  Results → {REPORT}/deeplog13_results.csv")
...
ax.set_title('DeepLog Training & Validation Loss — HDFS [Du2017]', fontweight='bold')
...
ax.set_title('DeepLog Val F1 During Training — HDFS [Du2017]', fontweight='bold')
ax.legend(); ax.grid(alpha=0.3); plt.tight_layout()
...
ax1.set_title('k Selection — Validation F1  [Du2017 Protocol]', fontweight='bold')
...
# =============================================================================
print("\n" + "=" * 70)
print("  ✅  FINAL RESULTS — DeepLog HDFS [Du2017]")
print("=" * 70)
print(f"\n  Vocab size   : {VOCAB_SIZE}  (train templates + PAD)")
print(f"  Window size  : {WINDOW_SIZE}  [Du2017 §5.1]")
...
print("    FIX 5:  groupby() loading (~50× faster than iterrows)")
