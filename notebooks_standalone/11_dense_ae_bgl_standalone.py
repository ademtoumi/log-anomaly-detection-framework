# =============================================================================
# STANDALONE NOTEBOOK — Dense Autoencoder on BGL (Fully Independent)
#
...
#
# ✅ ZERO dependencies — reads raw BGL_Drain.csv directly.
...
BASE_OUT = '/kaggle/working'               if KAGGLE else 'result/results_DenseAE_BGL'
...
print(f"{'Kaggle' if KAGGLE else 'Local'} | Dense AE on BGL — Standalone")

# ─────────────────────────────────────────────────────────────────────────────
# CELL 2 — Load BGL + Build TF-IDF Splits Inline
...
def ae_clustering_threshold_search(errors_all, dists_all, threshold_d, y_true, n_points=1000):
    """
    Search for reconstruction threshold combined with K-Means distance threshold.
    Anomaly if reconstruction error > threshold_e OR clustering distance > threshold_d
    """
...
if 'ae_done' not in ckpt:
    ...
    def ae_objective(trial):
        ...
        _, _, val_loss = train_autoencoder(
    ...
    print(f"\n  📊 TEST RESULTS — BGL AE + Clustering:")
    ...
    pd.DataFrame([metrics]).to_csv(f'{REPORT}/ae_bgl_results.csv', index=False)
    ...
    ax.set_title('Dense AE — Loss Curve (BGL)', fontweight='bold')
    ax.legend(); ax.grid(alpha=0.3); plt.tight_layout()
    plt.savefig(f'{REPORT}/ae_loss_bgl.png', dpi=300); plt.close()
    ...
    ax.set_title('Dense AE — Reconstruction Error Distribution (BGL)', fontweight='bold')
    ax.legend(); ax.grid(alpha=0.3); plt.tight_layout()
    plt.savefig(f'{REPORT}/ae_hist_bgl.png', dpi=300); plt.close()
    ...
    ax.set_title('Dense AE + Clustering CM — BGL', fontweight='bold')
    plt.tight_layout()
    plt.savefig(f'{REPORT}/ae_cm_bgl.png', dpi=300); plt.close()
    ...
    save_ckpt(ckpt)
    print(f"\n  ✅ Dense AE + Clustering BGL done in {time.time()-t0:.0f}s")
...
# ─────────────────────────────────────────────────────────────────────────────
print(f"\n{'='*60}")
print("  ✅ Dense AE + Clustering BGL STANDALONE — COMPLETE")
print(f"{'='*60}")
output_files = [
    (f'{BASE_OUT}/models/ae_{DS_KEY}_opt.pt',        'ae_bgl_opt.pt'),
    (f'{BASE_OUT}/models/ae_{DS_KEY}_pca.pkl',       'ae_bgl_pca.pkl'),
    (f'{BASE_OUT}/models/ae_{DS_KEY}_kmeans.pkl',    'ae_bgl_kmeans.pkl'),
    (f'{BASE_OUT}/models/ae_{DS_KEY}_threshold.json', 'ae_bgl_threshold.json'),
    (f'{REPORT}/ae_bgl_results.csv',                  'ae_bgl_results.csv'),
    (f'{REPORT}/ae_loss_bgl.png',                     'ae_loss_bgl.png'),
    (f'{REPORT}/ae_hist_bgl.png',                     'ae_hist_bgl.png'),
    (f'{REPORT}/ae_cm_bgl.png',                       'ae_cm_bgl.png'),
]
...
print(f"\n  📊 Results → {REPORT}/ae_bgl_results.csv")
print(f"{'='*60}")
