# =============================================================================
# STANDALONE NOTEBOOK 07 — CNN+BiLSTM Hybrid on HDFS (Fully Independent)
#
# Based on [Lu2018_LogCNN]      — Multi-scale 1D CNN (kernels [2,3,5]) captures
#   different n-gram log-event patterns at multiple scales.
# Based on [Zhang2019_LogRobust] — Attention over LSTM outputs focuses the model
#   on the most anomalous timesteps within a session.
# Based on [Guo2021_LogBERT]   — Bidirectional context is superior for offline
#   anomaly detection on HDFS sessions.
# Based on [Bekkouche2025_BiLSTM] — HDFS sessions grouped by BlockId; F1-optimal
#   threshold on VALIDATION SET ONLY. Test set touched exactly once.
#
# Architecture: Embedding → Multi-Scale CNN (kernels [2,3,5]) → BiLSTM
#               → Attention → Dropout → FC → Sigmoid
#
# ✅ ZERO dependencies — reads raw HDFS_Drain.csv directly.
# ✅ Builds sessions inline (chunked reading, BlockId extraction, vocab build).
# ✅ Vocab built from TRAIN sessions only — no val/test leakage.
# ✅ Temporal split 60/20/20 on sessions.
# ✅ F1-optimal threshold searched on VALIDATION probabilities.
# ✅ Test set evaluated ONCE at the end with the val-derived threshold.
# ✅ One dataset only (HDFS) → RAM stays safe on Kaggle.
# ✅ AMP + gradient clipping + CosineAnnealingWarmRestarts.
# ✅ Checkpoint system: skips completed steps on re-run.
# ✅ Seed 42 everywhere.
#
# Kaggle setup:
...
BASE_OUT = '/kaggle/working'               if KAGGLE else 'result/results_CNNBiLSTM_HDFS'
...
# Checkpoint — re-running skips completed steps
...
#
# Vocab built from TRAIN sessions only — no leakage [Bekkouche2025_BiLSTM].
...
if 'sessions_ready' not in ckpt:
    ...
    # Aggregation: block_id → {'events': list[str], 'label': int}
    ...
    print(f"\n  ✅ Total rows processed | Sessions: {n_sessions:,}")
    ...
    print(f"  Split → train={len(train_bids):,} | val={len(val_bids):,} "
          f"| test={len(test_bids):,}")

    # Build vocab from TRAIN sessions only — no data leakage
    ...
    # Keep all events seen ≥2 times in training
    vocab = {'<PAD>': 0, '<UNK>': 1}
    for event, cnt in event_counter.most_common():
        if cnt >= 2:
            vocab[event] = len(vocab)
    VOCAB_SIZE = len(vocab)
    print(f"  Vocab size: {VOCAB_SIZE:,} (events seen ≥2 times in TRAIN only)")
    del event_counter; gc.collect()

    # Encode sessions → padded int32 arrays
    ...
    print(f"  ✅ Sessions ready ({time.time()-t0:.0f}s)")
    ckpt['sessions_ready'] = True; save_ckpt(ckpt)

else:
    print("[CELL 2] ⏭️  Loading cached sessions ...")
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 3 — MultiScaleCNNBiLSTM Architecture
#
...
#   All branches are concatenated → fed into BiLSTM for temporal context.
#
...
class MultiScaleCNNBiLSTM(nn.Module):
    ...
    Pipeline:
      ...
      3. Branches are truncated to the same length and concatenated along
         the channel dimension → fed into the BiLSTM as a sequence.
      ...
      6. Dropout + FC → scalar logit (binary cross-entropy loss).
    """
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 4 — Threshold Search Utility
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# CELL 6 — Optuna Hyperparameter Search + Full Training
#
# Search space based on [Lu2018_LogCNN] + [Zhang2019_LogRobust]:
#   embed_dim  : [32, 64, 128]    — embedding dimensionality
#   cnn_filters: [32, 64, 128]    — filters per CNN kernel
#   hidden_size: [64, 128, 256]   — BiLSTM hidden units (per direction)
#   num_layers : 1–3              — BiLSTM depth
#   dropout    : 0.1–0.5
#   lr         : log[1e-4, 5e-3]  — AdamW learning rate
...
# ─────────────────────────────────────────────────────────────────────────────
if 'done' in ckpt:
    print("⏭️  CNN+BiLSTM already done (checkpoint). Skipping training.")
else:
    print(f"\n{'='*65}")
    print(f"  🔬 HDFS CNN+BiLSTM HYBRID — MULTI-SCALE OPTIMIZATION")
    print(f"{'='*65}")
    t0_total = time.time()
    ...
    print(f"  🔍 Optuna (20 trials, timeout=1200s) ...")
    study.optimize(objective, n_trials=20, timeout=1200)
    best_params = study.best_params
    print(f"  🏆 Best params: {best_params}")
    print(f"  🏆 Best val F1 (Optuna): {study.best_value:.4f}")

    # Full training with best hyperparameters
    print(f"\n  🚀 Full training (50 epochs, patience=12) ...")
    ...
    print(f"\n  💾 Saved: cnn_bilstm_hdfs_opt.pt | cnn_bilstm_hdfs_config.json "
          f"| cnn_bilstm_hdfs_results.csv")

    # ── Plots ─────────────────────────────────────────────────────────────────
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(12, 4))
    ax1.plot(losses, 'b-o', markersize=3)
    ax1.set_title('Training Loss — HDFS CNN+BiLSTM', fontweight='bold')
    ...
    ax2.set_title('Validation F1 (optimal threshold) — HDFS CNN+BiLSTM',
                  fontweight='bold')
    ...
    plt.title('ROC — HDFS CNN+BiLSTM (Standalone)', fontweight='bold')
    ...
    ax.set_title('CM — HDFS CNN+BiLSTM (Standalone)', fontweight='bold')
    ...
    print(f"\n  ✅ CNN+BiLSTM HDFS Standalone complete ({time.time()-t0_total:.0f}s)")

# ─────────────────────────────────────────────────────────────────────────────
# CELL 7 — Verification Block
# ─────────────────────────────────────────────────────────────────────────────
print(f"\n{'='*65}")
print("  ✅ CNN+BiLSTM HDFS STANDALONE — COMPLETE")
print(f"{'='*65}")
...
print(f"\n  📊 Results CSV  → {REPORT}/cnn_bilstm_hdfs_results.csv")
print(f"  🧠 Model state  → {MODEL_DIR}/cnn_bilstm_hdfs_opt.pt")
print(f"  ⚙️  Config JSON  → {MODEL_DIR}/cnn_bilstm_hdfs_config.json")
print(f"\n  Paper citations:")
print(f"    [Lu2018_LogCNN]        — Multi-scale CNN (kernels 2,3,5)")
print(f"    [Zhang2019_LogRobust]  — Attention over all BiLSTM timesteps")
print(f"    [Guo2021_LogBERT]      — Bidirectional context for offline analysis")
print(f"    [Bekkouche2025_BiLSTM] — F1-optimal threshold on VAL, pos_weight")
print(f"\n  KEY CHANGE vs original:")
print(f"    ❌ Old: fixed threshold=0.5 for val early-stopping AND test eval")
print(f"    ❌ Old: label != '-' for anomaly detection (HDFS uses 'Normal')")
print(f"    ✅ New: threshold searched on val probs, applied once to test")
print(f"    ✅ New: label != 'Normal' for HDFS anomaly detection (correct)")
print(f"    ✅ New: vocab built from TRAIN only, MAX_SEQ_LEN=75 (consistent)")
