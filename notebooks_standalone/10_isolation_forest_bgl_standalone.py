...
# K-Means + Isolation Forest on BGL dataset (fully standalone)
# =============================================================================
# Papers:
...
OUTPUT_DIR = '/kaggle/working' if os.path.exists('/kaggle') else 'result/results_IF_BGL'
DS_KEY     = 'bgl'
...
print(f"{'='*60}")
print(f"  K-Means + Isolation Forest – BGL (standalone)")
print(f"{'='*60}")
...
def f1_threshold_search(scores_val, y_val, n_thresholds=200):
    ...
    We negate scores so that higher = more anomalous, then threshold.
    """
    neg_scores = -scores_val          # higher neg_score → more anomalous
...
if 'if_done' not in ckpt:
    ...
    scores_val        = final_if.decision_function(X_val)
    best_thr, val_f1  = f1_threshold_search(scores_val, y_val)
    ...
    y_score     = -scores_test   # higher = more anomalous
...
ax.set_title(f'K-Means + iForest BGL – Confusion Matrix\nF1={test_f1:.4f}')
for i in range(2):
    for j in range(2):
...
ax.set_title('ROC – K-Means + iForest BGL')
ax.legend()
plt.tight_layout()
...
ax.set_title('K-Means + iForest Score Distribution – BGL')
ax.legend()
plt.tight_layout()
...
# =============================================================================
print('\n' + '='*60)
print('CLASSIFICATION REPORT – K-Means + iForest BGL')
print('='*60)
